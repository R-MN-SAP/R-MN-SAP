...
void build_obj_min_sum(instance* inst)
{
	inst->rcnt = 1;
	inst->nzcnt = (int)(1 + (int)((pow(inst->n_individuals, 2))* inst->n_groups));
...
void build_MILP_min_sum(instance* inst) {
	...
	build_obj_min_sum(inst);

	cout << "Constraint 'min sum' inserted \n";
	...
	//build_constr_symmetry_breaking(inst);
	//cout << "Constraint 'constr_symmetry_breaking' inserted \n";

	if (inst->MagnantiW >= 1) 
	{
		build_constr_min_sum_first_cut(inst);
	...
	inst->status = CPXwriteprob(inst->env_MILP, inst->lp_MILP, "LP_MODEL.lp", NULL);
...
void solve_MILP_min_sum(instance* inst) {
	...
	cout << endl;
	cout << "-------------------------------------------" << endl;
	cout << "Solving the MIN-SUM problem using B&C (Initial MG cut = " << inst->MagnantiW << ")" << endl;
	cout << "-------------------------------------------" << endl;
