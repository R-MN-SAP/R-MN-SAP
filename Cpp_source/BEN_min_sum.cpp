...
int position_BEN_min_sum_eta(instance* inst)
{
	return (int)((inst->n_individuals * inst->n_groups));
}


int position_BEN_min_sum_x(instance* inst, int i, int c)
{
...
void build_constr_BEN_min_sum_c1(instance* inst)
{
	for (int i = 0; i < inst->n_individuals; i++)
	{
		...
		{
			inst->rmatind[c] = position_BEN_min_sum_x(inst, i, c);
...
void build_constr_BEN_min_sum_tau(instance* inst)
{
	for (int c = 0; c < inst->n_groups; c++)
	{
		...
		for (int i = 0; i < inst->n_individuals; i++) {
			...
			inst->rmatind[i] = position_BEN_min_sum_x(inst, i, c);
...
void build_constr_BEN_min_sum_homogeneiry(instance* inst)
{
		...
		for (int c0 = 0; c0 < inst->n_groups; c0++)
		{
			for (int c1 = 0; c1 < inst->n_groups; c1++)
			{
				...
				for (int i = 0; i < inst->n_individuals; i++) {
					...
					inst->rmatind[count_var] = position_BEN_min_sum_x(inst, i, c0);
				...
				for (int i = 0; i < inst->n_individuals; i++) {
					...
					inst->rmatind[count_var] = position_BEN_min_sum_x(inst, i, c1);
...
void build_constr_BEN_min_sum_first_cut(instance* inst)
{
	inst->rcnt = 1;
	...
	inst->rmatind[counter] = position_BEN_min_sum_eta(inst);
	...
	for (int c = 0; c < inst->n_groups; c++)
	{
		for (int i = 0; i < inst->n_individuals; i++)
		{
			...
			inst->rmatind[counter] = position_BEN_min_sum_x(inst, i, c);
...
void build_constr_BEN_min_sum_symmetry_breaking(instance* inst)
{
	int count_var;
	...
	for (int c = 0; c < inst->n_groups - 1; c++)
	{
		...
		for (int i = 0; i < inst->n_individuals; i++) {
			...
			inst->rmatind[count_var] = position_BEN_min_sum_x(inst, i, c);
		...
		for (int i = 0; i < inst->n_individuals; i++) {
			...
			inst->rmatind[count_var] = position_BEN_min_sum_x(inst, i, c + 1);
...
void build_constr_BEN_min_sum_symmetry_breaking_QA(instance* inst)
{
	int count_var;
	double RHS;
...
void build_constr_BEN_min_sum_Taylor_QA(instance* inst)
{
	inst->rcnt = 1;
	inst->nzcnt =  1 + (inst->n_groups * inst->n_individuals);
...
/*****************************************************************/
int CPXPUBLIC mycutcallback_IMAST_BEN_min_sum_single(CPXCENVptr env, void* cbdata, int wherefrom, void* cbhandle, int* useraction_p)
/*****************************************************************/
{
	...
	for (int i = 0; i < inst->n_individuals; i++)
	{
		for (int c = 0; c < inst->n_groups; c++)
		{
			inst->x_ic[i][c] = (double)inst->sol_callback[position_BEN_min_sum_x(inst, i, c)];
		}
	}
	...
	int counter;
	double eta_val = (double)inst->sol_callback[position_BEN_min_sum_eta(inst)];

	counter = 0;
	dual2 = 0.0;

	if (inst->external_sol >= 1)
	{
		get_zeta_MW(inst);
	}
...
/*****************************************************************/
int CPXPUBLIC mycutcallback_IMAST_BEN_min_sum(CPXCENVptr env, void* cbdata, int wherefrom, void* cbhandle, int* useraction_p)
/*****************************************************************/
{
	(*useraction_p) = CPX_CALLBACK_DEFAULT;
...
/*****************************************************************/
{
	...
	inst->cut_BEN_Tay_rmatind[counter] = position_BEN_min_sum_eta(inst);
	...
	for (int c = 0; c < inst->n_groups; c++)
	{
		for (int i = 0; i < inst->n_individuals; i++)
		{
			...
			inst->cut_BEN_Tay_rmatind[counter] = position_BEN_min_sum_x(inst, i, c);
...
void build_BEN_min_sum(instance* inst) {
	...
	if (inst->UB_heur >= 1)
	{
		inst->ub[counter] = (double)inst->sol_val_UP[0];
	}
	else
	{
		inst->ub[counter] = (double)pow(inst->ccnt, 2);
	}
	...
	build_constr_BEN_min_sum_c1(inst);

	cout << "Constraint 'c1' inserted \n";

	build_constr_BEN_min_sum_tau(inst);

	cout << "Constraint 'constr_tau' inserted \n";

	build_constr_BEN_min_sum_homogeneiry(inst);

	cout << "Constraint 'constr_homogeneiry' inserted \n";

	if (inst->MagnantiW >= 1) {
		build_constr_BEN_min_sum_first_cut(inst);
		cout << "Constraint 'build_constr_BEN_min_sum_first_cut' inserted \n";
	}
...
void solve_BEN_min_sum(instance* inst) {
		
	cout << endl;
	cout << "-------------------------------------------" << endl;
	cout << "Solving the MIN-SUM problem using Benders decomposition (Initial MG cut = " << inst->MagnantiW << ")" << endl;
	cout << "-------------------------------------------" << endl;
	...
	inst->nzcnt_BEN = 1 + (inst->n_individuals* inst->n_groups);
	...
	// if (status != 0) {
	//	 printf("error for CPX_PARAM_VARSEL\n");
	// }
	...
	inst->status = CPXsetlazyconstraintcallbackfunc(inst->env_MILP, mycutcallback_IMAST_BEN_min_sum, inst);
...
//#endif
	...
	for (int s = 0; s < inst->n_sol; s++)
	{
		for (int c = 0; c < inst->n_groups; c++)
		{
			for (int i = 0; i < inst->n_individuals; i++)
			{
				inst->X_ic[s][i][c] = inst->x[position_BEN_min_sum_x(inst, i, c)];
				inst->Heur_X_ic[s][i][c] = inst->x[position_BEN_min_sum_x(inst, i, c)];
			}
		}
	}
...
void clean_BEN_min_sum(instance* inst) {
